...
export function renderExam() {
  ...
  app().innerHTML = `
    <div class="view" style="text-align:center;padding-top:var(--sp-6)">
      <div style="display:flex;justify-content:center;margin-bottom:var(--sp-3);color:var(--blue)">${icon('TARGET', 52)}</div>
      ...
      <div style="display:flex;flex-direction:column;gap:var(--sp-3);max-width:320px;margin:0 auto">
        <button class="btn btn-primary btn-full" style="font-size:var(--text-md);padding:var(--sp-4);gap:8px" id="start-exam">${icon('ROCKET',18)} Démarrer</button>
...
function startExam() {
  ...
  DECKS.forEach(deck => {
    shuffle(deck.cards).slice(0, perDeck)
      .forEach(c => allCards.push({ ...c, deckId: deck.id, deckColor: deck.color, deckIcon: deck.icon }));
  });
  const queue = shuffle(allCards).slice(0, EXAM_COUNT);
  let idx = 0, correct = 0, results = [], timer = null, remaining = EXAM_DURATION;

  function tick(rem) {
    remaining = rem;
    const el  = document.getElementById('exam-timer');
    if (el) { el.textContent = formatTime(rem); if (rem <= 300) el.style.color = 'var(--red)'; }
  }
  function timeUp() { showToast('Temps écoulé !', 'warning', 3000); finishExam(queue, results, correct); }
  function next() {
    ...
    <div class="view" style="display:flex;flex-direction:column;align-items:center">
      <div style="width:100%;max-width:460px;margin-bottom:var(--sp-3)">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:var(--sp-2)">
          ...
          <span id="exam-timer" style="font-family:var(--font-display);font-weight:700;font-size:var(--text-lg);font-variant-numeric:tabular-nums">${formatTime(remaining)}</span>
...
function finishExam(queue, results, correct) {
  ...
  results.forEach(r => { if (byDeck[r.deckId]) { byDeck[r.deckId].total++; if (r.isCorrect) byDeck[r.deckId].correct++; } });

  const resultIcon = pct >= 80 ? icon('TROPHY', 52) : pct >= 50 ? icon('CHECK_CIRCLE', 52) : icon('DECKS', 52);
  ...
  app().innerHTML = `
    <div class="view" style="text-align:center">
      ...
      <div style="display:flex;flex-direction:column;gap:var(--sp-3);max-width:320px;margin:0 auto">
        <button class="btn btn-primary btn-full" id="retry-exam" style="gap:8px">${icon('REFRESH',16)} Réessayer</button>
        <button class="btn btn-secondary btn-full" id="home-exam" style="gap:8px">${icon('HOME',16)} Accueil</button>
      </div>
    </div>
  `;
